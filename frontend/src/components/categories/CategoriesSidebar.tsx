...
export const CategoriesSidebar = (props) => {
  ...
  let clearButton = document.getElementsByClassName("clear-selected-button");
        ...
        <ul className="categories-featured__list">
          {categories.map((category) => (
            <li key={category[0]} className="categories-featured__item">
              <Link
                className="categories-featured__item-link"
                to={`/categories/${category[1]}/1${location.search}`}
              >
                {category[0]}
              </Link>
            </li>
      ...
      <div>
        ...
        <div className="categories-sidebar-filters">
          ...
          <Link
            ...
            to={`?search=${query.get("search")}&tags=${selected.map(
              (tag) => `${tag.label}`
            )}`}
          >
            Apply filters
